...
class Nav2RoundTripTest(DockerROSTest):
    ...
    async def _reset_map(self):
        self.log("\n--- Checking fresh Cartographer session ---", color="yellow")
        # _start_container() has just restarted the ROS container, so Cartographer
        # starts with an empty in-memory pose graph. Cartographer intentionally does
        # not expose a generic reset service. Verify its trajectory/state services
        # instead of calling an unrelated service containing the word "reset".
        services = set(
            line.strip()
            for line in (await self._docker_exec("ros2 service list", timeout=10)).splitlines()
            if line.strip()
        )
        required = {"/write_state", "/finish_trajectory", "/start_trajectory"}
        missing = required - services
        assert not missing, f"Missing Cartographer services: {sorted(missing)}"
        self.log("Cartographer service API is available", color="green")
...
# Keep the original RoboMarvel home page and add only the requested maze
# controls.  The ROS node exposes a small API through Docker port 8090; the
# browser never needs to open a replacement dashboard.
MAZE_API_BASE = "http://127.0.0.1:8090"
SEARCH_API_BASE = "http://127.0.0.1:8091"


async def maze_api(path: str, method: str = "GET") -> dict:
...
async def search_api(
    ...
    def request_sync() -> dict:
        ...
        if payload is not None:
            ...
            data = b"{}"
        req = urllib_request.Request(
            SEARCH_API_BASE + path, data=data, method=method, headers=headers
        )
        with urllib_request.urlopen(req, timeout=2.0) as response:
            return json.loads(response.read().decode("utf-8"))
...
async def welcome_page(request: Request):
    ...
    with ui.column().classes(
        "items-center justify-center w-full min-h-screen gap-2 py-6"
    ):
        ui.label(f"{HOSTNAME.upper()}").classes("text-h2 font-mono")
        ui.label(f"IP: {ip}").classes("text-h6 font-mono mb-5")

        # Original environment and links are intentionally preserved.
        with ui.card().classes("w-100 p-6 shadow-lg"):
            for label, icon, url in LINKS:
                with ui.link(
                    ...
                    "bg-red-900 text-white no-underline rounded w-full p-2"
                ):
                    with ui.row().classes("items-center justify-center"):
                        ui.icon(icon, size="md")
                        ui.label(label.upper())

        # Only this card is new.
        with ui.card().classes("w-100 p-6 shadow-lg"):
            with ui.row().classes("items-center w-full"):
                ui.icon("explore", size="md")
                ui.label("ЛАБИРИНТ").classes("text-h6 font-bold col")
                state_label = ui.badge("OFFLINE")

            message_label = ui.label("Проверка связи с ROS…").classes(
                "text-body2 w-full"
            )
            counters_label = ui.label("целей: 0 · ошибок: 0").classes(
                "text-caption text-grey-5"
            )

            async def refresh_maze_status() -> None:
                try:
                    status = await maze_api("/api/maze/status")
                    active = bool(status.get("active"))
                    state_label.set_text(str(status.get("state", "UNKNOWN")))
                    message_label.set_text(str(status.get("message", "")))
                    counters_label.set_text(
                        "целей: "
                        f"{status.get('completed_goals', 0)} · ошибок: "
                        f"{status.get('failed_goals', 0)} · границ: "
                        f"{status.get('frontier_candidates', 0)}"
                    )
                    if active:
                    ...
                    state_label.set_text("OFFLINE")
                    message_label.set_text(f"Нет связи с maze_explorer: {exc}")
                    start_button.disable()
                    stop_button.disable()

            async def start_maze() -> None:
                try:
                    result = await maze_api("/api/maze/start", "POST")
                    ui.notify(result.get("message", "Запуск принят"), type="positive")
                except Exception as exc:
                    ui.notify(f"Не удалось запустить: {exc}", type="negative")
                await refresh_maze_status()

            async def stop_maze() -> None:
                try:
                    result = await maze_api("/api/maze/stop", "POST")
                    ui.notify(result.get("message", "Остановка принята"), type="warning")
                except Exception as exc:
                    ui.notify(f"Не удалось остановить: {exc}", type="negative")
                await refresh_maze_status()

            with ui.row().classes("w-full justify-center gap-3"):
                start_button = ui.button(
                    "ЛАБИРИНТ", icon="play_arrow", on_click=start_maze
                ).classes("bg-red-900")
                stop_button = ui.button(
                    "СТОП", icon="stop", on_click=stop_maze
                ).props("outline")
                stop_button.disable()

            ui.timer(1.0, refresh_maze_status)
            await refresh_maze_status()

        with ui.card().classes("w-100 p-6 shadow-lg"):
            with ui.row().classes("items-center w-full"):
                ui.icon("radar", size="md")
                ui.label("ПОИСК ОБЪЕКТА").classes("text-h6 font-bold col")
                search_state_label = ui.badge("OFFLINE")
            ...
            search_message_label = ui.label("Проверка связи с менеджером поиска…").classes(
                "text-body2 w-full"
            )
            ui.label(
                "Голос временно отключён — используйте кнопку «НАЙТИ»"
            ).classes("text-caption text-grey-5")

            async def refresh_search_status() -> None:
                try:
                    status = await search_api("/api/search/status")
                    active = bool(status.get("active"))
                    low_battery = bool(status.get("low_battery"))
                    search_state_label.set_text(str(status.get("state", "UNKNOWN")))
                    search_message_label.set_text(str(status.get("message", "")))
                    if active or low_battery:
                        search_start_button.disable()
                    else:
                        search_start_button.enable()
                    if active:
                        search_stop_button.enable()
                        target_select.disable()
                    else:
                        search_stop_button.disable()
                        target_select.enable()
                except (OSError, ValueError, urllib_error.URLError) as exc:
                    search_state_label.set_text("OFFLINE")
                    search_message_label.set_text(f"Нет связи с поиском: {exc}")
                    search_start_button.disable()
                    search_stop_button.disable()

            async def start_search() -> None:
                try:
                    result = await search_api(
                        "/api/search/start", "POST", {"target": target_select.value}
                    )
                    ui.notify(result.get("message", "Запуск принят"), type="positive")
                except Exception as exc:
                    ui.notify(f"Не удалось запустить поиск: {exc}", type="negative")
                await refresh_search_status()

            async def stop_search() -> None:
                try:
                    result = await search_api("/api/search/stop", "POST")
                    ui.notify(result.get("message", "Остановка принята"), type="warning")
                except Exception as exc:
                    ui.notify(f"Не удалось остановить поиск: {exc}", type="negative")
                await refresh_search_status()

            with ui.row().classes("w-full justify-center gap-3"):
                search_start_button = ui.button(
                    "НАЙТИ", icon="search", on_click=start_search
                ).classes("bg-red-900")
                search_stop_button = ui.button(
                    "СТОП", icon="stop", on_click=stop_search
                ).props("outline")
                search_stop_button.disable()

            ui.timer(1.0, refresh_search_status)
            await refresh_search_status()
