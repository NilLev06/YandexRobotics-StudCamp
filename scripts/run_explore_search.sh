#!/usr/bin/env bash
# Launch the frontier-driven exploration wrapper around object_search.py,
# without modifying the main rover tree. Same safety preflights and launch
# conventions as run_object_search.sh -- see OBJECT_SEARCH.md.
...
usage() {
  cat <<'EOF'
Usage: run_explore_search.sh [launcher options] [behavior options]

Launcher options:
  --wav FILE           WAV played only after exploration is exhausted with no find
...
Useful behavior options:
  ...
  --max-cycles N         Search-circle + frontier-drive cycles before giving
                          up (default: 8)
  --max-runtime-s S      Overall time budget in seconds (default: 1200)

All other arguments are passed to explore_search.py.
EOF
}
...
stop_behavior() {
  interrupted=1
  echo "Stopping exploration container..." >&2
  docker stop --time 8 "$CONTAINER_NAME" >/dev/null 2>&1 || true
}
trap stop_behavior INT TERM HUP

echo "Launching guarded exploration search for '$TARGET_CLASS'. Motion remains disabled unless every safety gate passes."
docker run --rm --init \
  --name "$CONTAINER_NAME" \
  --network "$NETWORK" \
  --read-only \
...
case "$STATUS" in
  0)
    echo "Exploration search completed: '$TARGET_CLASS' was confirmed."
    ;;
  2)
    ...
    ;;
  *)
    echo "Exploration search stopped with status $STATUS; no no-find audio was played." >&2
    ;;
esac
