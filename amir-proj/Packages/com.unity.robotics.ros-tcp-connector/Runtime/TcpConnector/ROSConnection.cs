// Modified locally for the GFS-X physical-robot transport.
// See GFSX_PATCH.md at the package root for provenance and patch details.
using System;
...
namespace Unity.Robotics.ROSTCPConnector
{
    public class ROSConnection : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            {
                Debug.LogError(
                    "Only one ROSConnection may run at a time. Disabling the duplicate on " +
                    gameObject.name + ".");
                enabled = false;
                Destroy(this);
                return;
        ...
        public void Connect()
        {
            ...
            {
                // Connect() used to start another background loop every time
                // it was called. The older loop then became impossible to
                // cancel and could keep publishing stale safety commands.
                if (m_ConnectionThreadCancellation != null)
                    return;

                cancellation = new CancellationTokenSource();
                m_ConnectionThreadCancellation = cancellation;
            }
            ...
            Task.Run(async () =>
            {
                ...
                finally
                {
                    ...
                    {
                        if (ReferenceEquals(
                                m_ConnectionThreadCancellation,
                                cancellation))
                        {
        ...
        public void Disconnect()
        {
            CancellationTokenSource cancellation;
            lock (m_ConnectionLifecycleLock)
                cancellation = m_ConnectionThreadCancellation;
            ...
            CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    // Robot-control messages are small and time-sensitive.
                    // Disable Nagle batching so heartbeats and zero-stop
                    // commands are not delivered in delayed bursts.
                    client.NoDelay = true;
