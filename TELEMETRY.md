...
- **Измеренная скорость** (`/hardware/odom`) — что он *реально* делает по
  колёсным энкодерам. Разница между ними — признак пробуксовки/сопротивления.
...
скорость на дашборде считается из одометрии (колёсные энкодеры), а не из IMU
— это единственный сейчас достоверный источник.
