...
Wi-Fi требует, чтобы в `docker-compose.dashboard.yaml` был проброшен
`/proc/net/wireless` хоста как read-only (уже настроено) — без этого
контейнер видит только свою изолированную сеть без `wlan0`.
