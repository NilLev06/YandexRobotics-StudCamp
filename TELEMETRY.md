...
Линейную скорость по IMU показать нельзя: акселерометр на этом роверe не
подключён (`hwnode.py` явно помечает `linear_acceleration_covariance[0] = -1`
— "не использовать"), поэтому скорость берётся из одометрии, а не из IMU.
