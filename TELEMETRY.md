...
Показывает: батарею (В), температуру CPU, нагрузку CPU (load average за 1
минуту) и занятую память (%), клиренс лидара **и дальний край** (самая
дальняя видимая точка — не только ближайшее препятствие), свободное место
на диске, Wi-Fi (нужен доступ к `/proc/net/wireless` хоста — см. ниже),
что видит YOLO прямо сейчас (классы + уверенность), статус текущей цели
навигации (`/search/status`, публикует `object_search.py`).
...
= -1` прямым текстом помечает поле как "не использовать". EKF (`ekf.yaml`)
тоже не берёт ускорение из IMU, только угловую скорость по оси Z. Поэтому
...
Wi-Fi требует одноразовой установки на роверe (docker/runc не даёт
монтировать файлы из `/proc` напрямую в контейнер — только обычные файлы,
поэтому маленький systemd-таймер на хосте раз в 2 секунды читает
`/proc/net/wireless` и пишет значение в `/run/rover-wifi-dbm`, а уже его
дашборд монтирует read-only):

```sh
cd ~/z_boys
./scripts/install_wifi_probe.sh
```

Без этого шага карточка Wi-Fi просто покажет "—" (не ошибка, честно "нет
данных") — контейнер сам по себе видит только свою изолированную сеть без
`wlan0`.
